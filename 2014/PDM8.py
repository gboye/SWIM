...
def recoder(chaine):
    ...
    for n,element in enumerate(nodeIn):
        chaine=chaine.replace(element,nodeOut[n])
    return chaine
...
#                self.digraphe.add_edge(nDepart,nArrivee,weight=coef)
                if self.digraphe.has_edge(nArrivee,nDepart) and ("weight" in self.digraphe.node[nDepart]):
                    ...
                    self.graphe.add_node(renommer(nDepart),label=relabel(nDepart),weight=float(self.digraphe.node[nDepart]["weight"]),cell=paire.entree,tense=paire.entree.strip("0123456789."))
                    self.graphe.add_node(renommer(nArrivee),label=relabel(nArrivee),weight=float(self.digraphe.node[nArrivee]["weight"]),cell=paire.sortie,tense=paire.sortie.strip("0123456789."))
                    self.graphe.add_edge(renommer(nDepart),renommer(nArrivee),weight=poids)
